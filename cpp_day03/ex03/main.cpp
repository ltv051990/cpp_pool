...
int main(void)
{
	...
	SravTrap aa1("SravTrap VASIA");
	SravTrap bb1(aa1);
	bb1.setName("SravTrap HAVRILO");
	std::cout << "EnergyPoints of " << aa1.getName() << " = "  << aa1.getEnergyPoints() << std::endl;
	std::cout << "HitPoints of " << aa1.getName() << " = " << aa1.getHitPoints() << std::endl;

	aa1.rangedAttack("rashka");
	aa1.meleeAttack("parashka");
	aa1.takeDamage(100);
	aa1.takeEnergyDamage(100);
	std::cout << "EnergyPoints of " << aa1.getName() << " = "  << aa1.getEnergyPoints() << std::endl;
	std::cout << "HitPoints of " << aa1.getName() << " = " << aa1.getHitPoints() << std::endl;
	aa1.beRepaird(1000);
	aa1.energyRepaird(1000);
	std::cout << "EnergyPoints of " << aa1.getName() << " = "  << aa1.getEnergyPoints() << std::endl;
	std::cout << "HitPoints of " << aa1.getName() << " = " << aa1.getHitPoints() << std::endl;

	bb1.challengeNewcomer("enemy BTR-80");
	bb1.challengeNewcomer("enemy BMP");

	bb1.challengeNewcomer("enemy BDRDM");
	std::cout << "EnergyPoints of " << bb1.getName() << " = "  << bb1.getEnergyPoints() << std::endl;
	std::cout << "HitPoints of " << bb1.getName() << " = " << bb1.getHitPoints() << std::endl;

	bb1.energyRepaird(100);
	std::cout << "EnergyPoints of " << bb1.getName() << " = "  << bb1.getEnergyPoints() << std::endl;
	std::cout << "HitPoints of " << bb1.getName() << " = " << bb1.getHitPoints() << std::endl;

	std::cout << std::endl << ">>>>>>>>>>>>> WELCOME TO THE NINJIA PART <<<<<<<<<<<<<<" << std::endl << std::endl;

	NinjaTrap c1("HOMOSAKI");
	NinjaTrap c2("HOM");
	ClapTrap d1("BUMBUM");
	c1.setName("kAMIKADZE");

	c1.rangedAttack("rashka");
	c1.meleeAttack("parashka");
	c1.takeDamage(100);
	c1.takeEnergyDamage(100);
